...
**「〇〇ではなく、△△」「〇〇ではない。△△だ」「〇〇というより△△」**——
**否定を踏み台にして本命を出す構文を禁止する。**

**言いたいことは △△ だけを肯定文で書く。〇〇の側は書かない。**

読者が 〇〇 と誤解しそうな場合も、**「△△である」と書き切ることで対応する。**
否定形で予防線を張らない。
...
**単純な事実の否定は対象外。** 「彼は来なかった」「鍵は開いていない」「未来は見えない」。

## 事件の書き方

`events/` は年表の材料だが、**派手な出来事の記録簿にしない。**

**事件は、今の制度がなぜこうなっているかを説明するために書く。**

- 何が起きたか（**短く**）
- **何を証明したか**
- **何を変えたか**（制度、警戒基準、戦術、教育、思想）
- 今どう語られているか

戦闘の細部より、**その事件が社会に残した傷跡**を優先する。逆に言えば、**何も変えなかった出来事は `events/` に書かない**。

そして事件の中身は、**主題側のファイルへ配る**。制度が変わったなら該当する `concepts/` へ、組織が変わったなら `factions/` へ。事件ファイルに全部を集めると、**事件順に並んだだけの塊**になって、主題から引けなくなる。

**事件ファイルは薄く、リンクは多く。**

### 乱発しない

**背景にあるだけで十分に効く。** 授業で聞く、教師が例に出す、進路面談で脅し文句に使われる — その程度の登場でよい。
