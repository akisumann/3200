...
書く前に、聞く前に、**まず [`source/`](../source/README.md) を検索する。**

```
grep -rn "進路" source/
```

`source/` は**元資料**。加工していない原文を節ごとに分割してある。**`world/` は再構成した本筋**で、そこが正。

- **`source/` の文章を `world/` へそのまま持ち込まない**
- **食い違ったら `world/` が正**。原文より、後から確定した設定が優先する

これを飛ばして推測で書くと、資料に答えがあるのに間違ったものを書く。**実際に何度もやった。**
